...
script.on_event(defines.events.on_player_created, function(event)
	...
	if platform then
		...
		hub.insert({ name = "chemical-plant", count = 3 })
		hub.insert({ name = "oxy-fuel-furnace", count = 3 })
		hub.insert({ name = "pipe", count = 100 })
		hub.insert({ name = "gas-furnace", count = 1 })
		hub.insert({ name = "pipe-to-ground", count = 10 })
		hub.insert({ name = "long-handed-inserter", count = 10 })
	end
end)
